...
def load_config():
    ...
    store_type = os.getenv('MCARD_MANAGER_store_TYPE', 'sqlite')
    if store_type == 'sqlite':
        return AppSettings(
            store=SQLiteConfig(
                ...
                max_content_size=int(os.getenv('MCARD_MANAGER_MAX_CONTENT_SIZE', '10485760'))  # 10MB default
            ),
...
async def verify_api_key(x_api_key: str = Header(...), settings: AppSettings = Depends(load_config)):
...
class MCardAPI:
    ...
    def __init__(self, store: Optional[CardStore] = None, repository: Optional[CardStore] = None):
        ...
        self.hashing_service = get_hashing_service()
    ...
    async def get_card(self, hash_str: str) -> MCard:
        ...
        try:
            card = await self.store.get(hash_str)
            if not card:
...
async def create_card(card: CardCreate, repo: CardStore = Depends(get_store)):
    ...
    return CardResponse(
        ...
        hash=result.hash,
        g_time=result.g_time.isoformat() if result.g_time else None
    )
